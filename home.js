(() => {
  ...
  else{countdownLabel.textContent=days<=30?'已进入冲刺阶段':'距离考试';countdownValue.textContent=`${days} 天`;heroText.textContent=days<=30?'先处理日常高频、错题和到期内容；PDF原资料专区单独复习。':'先完成日常背诵，再按需要进入PDF加粗专题。'}
  ...
  if(pdfHost)pdfHost.innerHTML=`<div><span class="track-tag pdf">PDF独立专区</span><h3>计算机 PDF原资料 01—10</h3><p>只背黑色加粗内容，普通字只了解；不进入今日任务。</p><div class="bar"><i style="width:${ps.pct}%"></i></div><small>已完成 ${ps.completed}/${ps.total} 个PDF模块 · 其中加粗必背 ${ps.mastered}/${ps.memorizeTotal}</small></div><a class="primary-btn" href="./knowledge.html?track=pdf">进入PDF原资料专区</a>`;
  const b=plan.block||plan.todayDue[0],host=V.$('#currentBlock');
  if(!b){host.innerHTML='<div class="empty">当前日常核心背诵块已经完成，可进入题库或PDF原资料专区。</div>';return}
