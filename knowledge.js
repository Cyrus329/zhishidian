(() => {
  ...
  function cardHtml(b){
    ...
    return `<article class="compact-block-row ${isPdf?'pdf-block-row':'daily-block-row'}"><div class="compact-block-main"><div class="compact-block-title"><h4>${V.esc(displayTitle(b))}</h4><span>${status}</span></div><p>${s.core[0]?V.esc(s.core[0]):'本模块没有加粗必背内容，只需阅读了解'}</p><div class="mini-progress"><i style="width:${b.progress}%"></i></div><div class="compact-meta">${isPdf?'<span class="track-tag pdf">PDF原资料</span>':'<span class="track-tag daily">日常知识点</span>'}<span>必背 ${must}条</span><span>了解 ${understand}项</span><span>课堂笔记 ${b.notes.length}</span><span>关联题 ${b.questions.length}</span></div></div><div class="compact-block-actions"><a class="primary-btn" href="./learn.html?block=${encodeURIComponent(b.key)}&track=${b.track}">${b.knowledge.length?(isPdf?'背这一份PDF':'开始这一块'):'阅读了解'}</a>${b.knowledge.length?`<a class="secondary-btn" href="./learn.html?block=${encodeURIComponent(b.key)}&track=${b.track}&mode=test">背完检验</a>`:''}<details><summary>查看细项</summary><div class="fine-item-list">${[...b.knowledge,...b.understand].map(i=>`<a href="./library.html?item=${encodeURIComponent(i.id)}">${V.esc(i.title)}</a>`).join('')}</div></details></div></article>`;
  }
  function render(){
