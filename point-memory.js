...
const STORE='zsb-knowledge-memory-b002';
...
function emptyState(){return{version:2,progress:{},currentId:POINTS[0]?.id||'',history:[],settings:{tier:'core',subject:'',day:'',chapter:'',scope:'new',practice:'cloze'},time:{totalSeconds:0,todayDate:todayKey(),todaySeconds:0,bySubject:{},byDay:{}},daily:{},updatedAt:nowIso()}}
...
function progress(id){return state.progress[id]||{status:'new',stage:0,remember:0,fuzzy:0,forgot:0,attempts:0,streak:0,lastAt:'',nextDue:'',updatedAt:''}}
...
function parsePrompt(s){
 const parts=String(s||'').split(/(\{\{.*?\}\})/g);return parts.map(part=>{const m=part.match(/^\{\{(.*?)\}\}$/);return m?`<span class="blank" title="${esc(m[1])}">${esc(m[1])}</span>`:esc(part)}).join('')
}
function answerDetails(p){
 ...
 return `<div class="answer-main">${esc(p.point)}</div><div class="answer-extra"><b>关键词：</b>${esc((p.clozeAnswers||[]).join('、')||p.shortHint||'—')}<br><b>来源：</b>${esc(p.importLabel)}｜${esc(p.sourceTitle)}${extra}</div>`
}
function cardHtml(p,focus=false){
 ...
 const prompt=practice==='recall'?`<div class="recall-prompt"><small>请完整回忆</small><br>${esc(p.sourceTitle)}<br><span style="font-size:14px;color:#6c778d">${esc(p.chapter)}</span></div>`:`<div class="recall-prompt">${parsePrompt(p.clozePrompt)}</div>`;
 ...
 return `<div class="previous-point">${prev?`上一个：${esc(prev.sourceTitle)}｜${esc(prev.point)}`:'从第一个知识点开始'}</div>
...
function scheduleFor(status,old){const now=Date.now();if(status==='remember'){const stage=Math.min((old.stage||0)+1,6),days=[0,1,3,7,15,30,60][stage]||60;return{stage,nextDue:new Date(now+days*DAY_MS).toISOString(),streak:(old.streak||0)+1}}if(status==='fuzzy')return{stage:Math.max(0,old.stage||0),nextDue:new Date(now+12*3600000).toISOString(),streak:0};return{stage:0,nextDue:new Date(now+10*60000).toISOString(),streak:0}}
...
function grade(status){
 ...
 state.history.push({id:p.id,status,at});state.history=state.history.slice(-100);const d=daily();d.actions++;d[status]++;if(!d.ids.includes(p.id))d.ids.push(p.id);sessionActions++;
 if(status==='forgot'){
...
function exportData(){const obj={type:'zsb-knowledge-memory-backup',version:2,exportedAt:nowIso(),state};const blob=new Blob([JSON.stringify(obj,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`知识点记忆备份-${todayKey()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500)}
